...
import gerenciador.GerenciadorGame;
...
public class TestGame {
	...
	private GerenciadorGame gerenciadorGame;
	private TipoItem tipo = TipoItem.JOGOVIDEOGAME;
	
	@Before
	public void criaMidia() {
		listaDeGames = new ArrayList<Game>();
		gerenciadorGame = new GerenciadorGame();
		
		
		g1 = new Game("FIFA 15", "", "01/01/2015", 69.90, 0, 10, Estado.NOVO, false, tipo, Console.PC, true);
		dlcs1 = new ArrayList<DLC>();
		...
		g2 = new Game("Battlefield 4", "", "07/10/2014", 120.0, 1, 9, Estado.NOVO, true, tipo, Console.PC, true, true, dlcs1);
		dlcs2 = new ArrayList<DLC>();
		...
		g3 = new Game("Assassin's Creed IV: Black Flag", "O novo jogo da aclamada franquia Assassin's Creed "
				+ "tem agora como palco a fatídica Era Dourada da Pirataria. ", "15/12/2014", 160.90, 0, 9.5, Estado.NOVO, false, tipo, Console.PS4, true, true, dlcs2);
	}
	...
	public void testPesquisar() {
		try {
			saves();
			/**
			 * Pesquisa pelo objeto completo. É usando as regras do equals para comparação entre os objetos 
			 * Retorna apenas o objeto a ser buscado caso seja encontrado
			 */
			Assert.assertEquals(null, gerenciadorGame.pesquisar(new Game("Nao existo", "", "", 0, 0, 0, Estado.NOVO, false, tipo, Console.XBOX360, false))); // deverá retonar null pois o objeto não existe na lista
			Assert.assertEquals(g1, gerenciadorGame.pesquisar(g1)); // deverá retornar o objeto m1, pois ele estar contido na lista

			/**
			 * Pesquisa pelo título do item 
			 * Retorna uma lista com todas as ocorrencias da busca ou retorna uma lista vazia caso não encontra nada
			 */
			List<Game> verif_encontrado = new ArrayList<Game>();
			Game g4 = new Game("FIFA 14", "", "12/11/2013", 90.0, 5, 10, Estado.DESGATADO, true, tipo, Console.PS3, true);
	...
	public void testAtualizar() {
		...
		try {
			saves();
			Game g_ori = new Game("Exito para ser atualizado", "", "", 0, 0, 0, Estado.NOVO, false, tipo, Console.XBOX360, false);
			g_ori.setId(153);
			gerenciadorGame.save(g_ori);
			
			Game g_update = new Game("Novo jogo - Update", "", "", 260.90, 10, 0, Estado.NOVO, false, tipo, Console.XBOX360, false);
			g_update.setId(153);
			...
			Game nao_salvo = new Game("Não existo...", "", "", 0, 0, 0, Estado.NOVO, false, tipo, Console.XBOX360, false);
	...
	public void testRemover() {
		try {
			...
			Assert.assertTrue(gerenciadorGame.listar().size() == 0 );
		} catch (PersonalCollectionsException e) {
			e.printStackTrace();
		}
	}
	...
	public void testEquals() {
		...
		Assert.assertTrue(!(new Game("Battlefield 4", "", "07/10/2014", 120.0, 1, 9, Estado.NOVO, true, tipo, Console.PC, true, true, dlcs1).equals(g2)));
	}
